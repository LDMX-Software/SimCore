...
int main(int argc, char* argv[]) {
    ...
    if (!table_file.is_open()) {
        std::cerr << "File '" << argv[1] << "' was not able to be opened." << std::endl;
        return 2;
    }

    ldmx::Parameters model;
    model.addParameter( "name"         , std::string("vertex_library") );
    model.addParameter( "library_path" , std::string("${DB_VERTEX_LIB_INSTALL}/mA.0.01") );
...
void printUsage() {
    std::cout << "Usage: print-dark-brem-xsec-table {xsec_table.csv}" << std::endl;
    std::cout << "     xsec_table.csv  (required) file to print table to" << std::endl;
}
