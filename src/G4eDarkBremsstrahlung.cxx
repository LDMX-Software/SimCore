...
namespace ldmx {
    ...
    G4double ElementXsecCache::get(G4double energy, G4double A, G4double Z) {
        unsigned int key = computeKey(energy,A,Z);
    ...
    void ElementXsecCache::stream(std::ostream& o) const {
        ...
        for (auto const& [ key , xsec ] : the_cache_ ) {
            int Z = key%MAX_A;
            int A = ((key-Z)/MAX_A)%MAX_E;
            int E = ((key-Z)/MAX_A-A)/MAX_E;
            o << A << "," << Z << "," << E << "," << xsec/CLHEP::picobarn << "\n";
        }
        o << std::endl;
    }

    unsigned int ElementXsecCache::computeKey(G4double energy, G4double A, G4double Z) const {
        unsigned int energyKey = energy;
        unsigned int AKey      = A; 
        unsigned int ZKey      = Z;
        return (energyKey*MAX_E+AKey)*MAX_A + ZKey;
    }
